...
"""
Version 2: Optimize data frame and storage. Save names as separate data set with identifier as unique run ID
asyncio/multiprocessing to speed up 
requests session
try data stream option on html request for slight speed increase
...
def run_formatter(rank_trim): #takes a rank_trim html slice, returns a df with run data
    ...
    #change data types
    df['keystone_run_id'] = pd.to_numeric(df['keystone_run_id'])
    df['keystone_team_id'] = pd.to_numeric(df['keystone_team_id'])
    df['mythic_level'] = pd.to_numeric(df['mythic_level'])
    df['clear_time_ms'] = pd.to_numeric(df['clear_time_ms'])
    df['keystone_time_ms'] = pd.to_numeric(df['keystone_time_ms'])
    df['num_chests'] = pd.to_numeric(df['num_chests'])
    df['time_remaining_ms'] = pd.to_numeric(df['time_remaining_ms'])
    df['num_modifiers_active'] = pd.to_numeric(df['num_modifiers_active'])
